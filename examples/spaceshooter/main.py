#!/usr/bin/env python
# Space Shooter - Original by tasdik (MIT License)
# Adapted for wasmcart _wc_frame callback mode
# Assets: Kenney's Space Shooter pack (CC-BY-3)
# Music: Kenney Music Loops (CC0)
...
font_name = None  # Use pygame default font
    ...
    def shoot(self):
        ...
        if now - self.last_shot > self.shoot_delay:
            ...
            if self.power >= 3:
                b1 = Bullet(self.rect.left, self.rect.centery)
                b2 = Bullet(self.rect.right, self.rect.centery)
                m1 = Missile(self.rect.centerx, self.rect.top)
                all_sprites.add(b1, b2, m1)
                bullets.add(b1, b2, m1)
                shooting_sound.play()
                missile_sound.play()
...
meteor_images = []
for name in ['meteorBrown_big1.png', 'meteorBrown_big2.png',
             'meteorBrown_med1.png', 'meteorBrown_med3.png',
             'meteorBrown_small1.png', 'meteorBrown_small2.png',
             'meteorBrown_tiny1.png']:
    meteor_images.append(pygame.image.load(path.join(img_dir, name)).convert())
...
for i in range(9):
    ...
    img2 = pygame.image.load(path.join(img_dir, f'sonicExplosion0{i}.png')).convert()
...
expl_sounds = [pygame.mixer.Sound(path.join(sound_folder, s))
...
def start_game():
    global all_sprites, mobs, bullets, powerups, player, score, state, death_explosion
    all_sprites = pygame.sprite.Group()
    mobs = pygame.sprite.Group()
    bullets = pygame.sprite.Group()
    powerups = pygame.sprite.Group()
...
def _wc_frame():
    global state, title_img, menu_timer, score, death_explosion
    ...
    if state == STATE_MENU:
        ...
        draw_text(screen, "SPACE SHOOTER", 48, WIDTH / 2, HEIGHT / 4)
        draw_text(screen, "Arrow keys to move, Space to fire", 22, WIDTH / 2, HEIGHT / 2)
        draw_text(screen, "Press ENTER to start", 28, WIDTH / 2, HEIGHT * 3 / 4)
        pygame.display.flip()
        return
    ...
    if state == STATE_GAMEOVER:
        ...
        draw_text(screen, "Press ENTER", 22, WIDTH / 2, HEIGHT * 3 / 4)
        pygame.display.flip()
        return
