...
class Geometry(object):
    ...
    def processAttribute(self, name):
        ...
        # convert to a numpy array and then to bytes for GL
        array = np.array( data["value"] ).astype( np.float32 )
        buf = array.ravel().tobytes()
    ...
    def setupVAO(self, shaderProgramID):
        ...
        for name, data in self.attributeData.items():
            ...
            if attributeVarID is not None and attributeVarID != -1:
                glEnableVertexAttribArray(attributeVarID)
                if data["type"] == "float":
                    glVertexAttribPointer(attributeVarID, 1, GL_FLOAT, False, 0, 0)
                elif data["type"] == "vec2":
                    glVertexAttribPointer(attributeVarID, 2, GL_FLOAT, False, 0, 0)
                elif data["type"] == "vec3":
                    glVertexAttribPointer(attributeVarID, 3, GL_FLOAT, False, 0, 0)
                elif data["type"] == "vec4":
                    glVertexAttribPointer(attributeVarID, 4, GL_FLOAT, False, 0, 0)
                else:
